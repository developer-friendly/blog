...
csrf_token="$(echo $output | jq -r '.ui.nodes[] | select(.attributes.name == "csrf_token") | .attributes.value')"
...
curl -XPOST -sS --cookie $cookie_jar --cookie-jar $cookie_jar \
  ...
  -d "{\"traits.email\":\"$email\",\"password\":\"$password\",\"csrf_token\":\"$csrf_token\",\"method\":\"password\"}" \
  $action
